...
import { storage, useIsDataReady, useLocalSetting, useLocalSettingMutable, usePendingAttachments, useRealtimeStatus, useSessionMessages, useSessionUsage, useSetting } from '@/sync/storage';
...
import { uploadSessionFile } from '@/sync/apiSessionFiles';
...
function SessionViewLoaded({ sessionId, session }: { sessionId: string, session: Session }) {
    ...
    // Image attachments (pending uploads, staged for next send)
    const pendingAttachments = usePendingAttachments(sessionId);
    const [attachmentThumbs, setAttachmentThumbs] = React.useState<Record<string, string>>({});
    const [isAttaching, setIsAttaching] = React.useState(false);
    const handlePickImage = React.useCallback(async () => {
        if (isAttaching) return;
        const auth = getCurrentAuth();
        if (!auth?.credentials) {
            Modal.alert(t('common.error'), 'Not authenticated');
            return;
        }
        try {
            const perm = await ImagePicker.requestMediaLibraryPermissionsAsync();
            if (!perm.granted) {
                Modal.alert(t('common.error'), t('errors.permissionDenied'));
                return;
            }
            ...
            storage.getState().addPendingAttachment(sessionId, uploaded);
            setAttachmentThumbs((prev) => ({ ...prev, [uploaded.fileId]: asset.uri }));
        } catch (err) {
    ...
    }, [isAttaching, sessionId]);
    const handleRemoveAttachment = React.useCallback((fileId: string) => {
        storage.getState().removePendingAttachment(sessionId, fileId);
